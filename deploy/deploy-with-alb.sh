...
# Script de despliegue para EU Chat Bridge con ALB
# Despliega en puerto 3001 (spainbingo sigue en 3000)
...
echo -e "${BLUE}🚀 Despliegue EU Chat Bridge con ALB${NC}"
echo "=========================================="
echo -e "${YELLOW}Servidor: ${EC2_HOST}${NC}"
echo -e "${YELLOW}Puerto Backend: ${BACKEND_PORT}${NC}"
echo -e "${YELLOW}ALB: ${ALB_DNS}${NC}"
echo -e "${YELLOW}spainbingo: Puerto 3000 (existente)${NC}"
echo -e "${YELLOW}EU Chat Bridge: Puerto 3001 (nuevo)${NC}"
echo ""
...
cat > deploy/ecosystem-alb.config.js << 'EOF'
module.exports = {
  apps: [{
...
scp_copy "deploy/ecosystem-alb.config.js" "ec2-user@${EC2_HOST}:~/${PROJECT_NAME}/"
...
ssh_exec "
    ...
    # Iniciar con PM2
    pm2 start ecosystem-alb.config.js
...
echo -e "   Antes de usar, ejecuta: ./deploy/configure-alb.sh"
echo -e "   Esto configurará el ALB para el puerto 3001"
